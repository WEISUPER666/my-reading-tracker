...
| 功能 | 说明 |
|------|------|
| **分类管理** | 支持创建、编辑、删除自定义分类，书籍录入和编辑时可选择分类 |
| **分类图标** | 支持为每个分类设置 Emoji 图标，提供丰富的图标选择器 |
...
| 功能 | 说明 |
|------|------|
| **平台管理** | 支持创建、删除自定义阅读平台，独立管理来源平台列表 |
| **默认平台** | 首次运行时自动创建默认平台（微信读书、喜马拉雅、本地文件、实体书） |
| **平台选择** | 录入阅读记录时可从平台列表中选择，也可手动输入 |
...
| `icon` | String (Nullable) | 分类图标（Emoji 或图片 URL） |
...
- 🏷️ **分类图标管理**：支持为每个分类设置 Emoji 图标，提供丰富的图标选择器
- 📱 **平台管理**：新增独立的平台管理功能，支持创建、删除自定义阅读平台
- 🔧 **分类更新接口**：新增 `PUT /api/categories/{category_id}` 接口，支持更新分类名称和图标
- 📋 **平台管理接口**：新增完整的平台 CRUD 接口
- 🗃️ **平台数据模型**：新增 `Platforms` 表，独立管理来源平台
- 🎨 **默认平台初始化**：首次运行时自动创建默认平台（微信读书、喜马拉雅、本地文件、实体书）
